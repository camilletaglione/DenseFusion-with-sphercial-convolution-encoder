...
#import cv2
from tools.encoder import Encodertorch
from lib.dataloder import Fetcher
from lib.pc_utils import load_depth, backproject, pc2sphericalmap
...
class DualPoseNet(object):
    def __init__(self, opts, sess):
        ...
        if self.opts.dataset == 'ycb':
            self.intrinsics = np.array(
                [[591.0125, 0, 322.525], [0, 590.16775, 244.11084], [0, 0, 1]])
        elif self.opts.dataset == 'linemod':
            self.intrinsics = np.array(
    ...
    def build_model(self):
        # model
        self.encoder = Encodertorch(self.opts, self.is_training, name='encoder')
    ...
    def setup_optimizer(self):
        ...
        all_update_ops = [op for op in tf.get_collection(tf.GraphKeys.UPDATE_OPS) if op.name.startswith("encoder")]
        all_tvars = [var for var in tf.trainable_variables() if var.name.startswith("encoder")]
        ...
        for epoch in range(1, self.opts.training_epoch+1):
            ...
            for batch_idx in range(fetchworker.num_batches):
                ...
                _, loss = self.sess.run(
                    [self.all_optimizers, self.loss], feed_dict=feed_dict)
