...
for now in range(0,2949):
    ...
    for idx in range(len(lst)):
        ...
        try:
            ...
            bb = np.array([rmin, rmax, cmin, cmax])
            ...
            img_masked1 = np.array(img)[:, :, :3]
            img_masked = np.transpose(img_masked1, (2, 0, 1))
            ...
            #print (img.size, np.max(img), np.min(img))
            #print (s,np.max(img_masked), np.min(img_masked))
            img_masked2 = np.zeros((s), dtype=int)
            for k in range(3):
                img_masked2[:,:,k] = img_masked[k,:,:]
            img_masked_array = Image.fromarray(img_masked2, 'RGB')
            save_path = "data/res2/masked_{}.png".format("%04d" % now)
            mask = Image.fromarray(mask_label)
            mask.save(save_path,'png')
        
            #np.savetxt(text_file, [bb], fmt='%.0f')
            #print('bbox',bb)
            #print('idx', idx)
            ...
            save_path2 = "data/res3/imgmasked_{}.png".format("%04d" % now)
            T = transforms.ToPILImage()
            imgm = img_masked.cpu()
            imagm = T(imgm[0])
            imagm.save(save_path2,'png')
            ...
            for ite in range(0, iteration):
                ...
                #print('frame {0}, pose r {1} t {2}'.format(idx,my_r,my_t))
                #print (my_pred.shape,  np.max(my_pred), np.min(my_pred))
                """ np.savetxt(text_file, [now], fmt='%d')
                np.savetxt(text_file, [idx], fmt='%d')
                np.savetxt(text_file, [ite], fmt='%d')
                np.savetxt(text_file, [my_r], fmt='%f')
                np.savetxt(text_file, [my_t], fmt='%f')
                np.savetxt(text_file, [bb], fmt='%.0f') """
                #print(my_r)
